#!/bin/bash
# chunk_gated_delta_rule_fwd_h 算子 UT 测试执行脚本
...
echo "========================================"
echo "chunk_gated_delta_rule_fwd_h 算子 UT 测试"
echo "========================================"
...
if ./chunk_gated_delta_rule_fwd_h_op_host_ut; then
...
echo "========================================"
DATA_DIR="${SCRIPT_DIR}/op_kernel/chunk_gated_delta_rule_fwd_h_data"
cd "${DATA_DIR}"
python3 gen_data.py
...
if [ -d "${BUILD_DIR}/../../../op_api" ]; then
    cd "${BUILD_DIR}/op_api"
    if ./chunk_gated_delta_rule_fwd_h_op_api_ut; then
        PASSED_TESTS+=("op_api")
    else
        FAILED_TESTS+=("op_api")
    fi
