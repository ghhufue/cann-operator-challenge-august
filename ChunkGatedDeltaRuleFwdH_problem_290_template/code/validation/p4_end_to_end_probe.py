#!/usr/bin/env python3
"""End-to-end probe: emulate the NPU kernel with P4 (4-way interleaved fp32)
manual matmul for MM1/MM2, and compare bit-exactly against the torch golden
outputs of the official_example case."""
import json
...
def mm_p4(a, b):
    """Manual bf16 matmul with 4-way interleaved fp32 accumulation.
    a: (M,K) bf16, b: (K,N) bf16 -> (M,N) bf16"""
    M, K = a.shape
    N = b.shape[1]
...
def run_reference_p4(k, w, u, g, initial_state, cu_seqlens, chunk_indices):
    ...
    for sequence in range(sequence_count):
        ...
        for head in range(value_heads):
            ...
            for chunk in range(sequence_chunks):
                ...
                # MM1 with P4 manual matmul
                ws = mm_p4(w_sel, state)
                ws_fp16 = torch.nan_to_num(ws.to(torch.float16), nan=0.0,
                                           posinf=torch.inf, neginf=-torch.inf)
                ...
                # MM2 with P4 manual matmul (k_sel^T @ v_decay_bf16)
                update = mm_p4(k_sel.transpose(-1, -2).contiguous(), v_decay_bf16)
                update_fp16 = torch.nan_to_num(update.to(torch.float16), nan=0.0,
                                               posinf=torch.inf, neginf=-torch.inf)
